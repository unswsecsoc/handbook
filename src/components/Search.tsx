...
function Search({ tag }: SearchInit) {
  ...
  const posts = query.length > 0 ? fuse.search(query).map((post) => post.item) : allPosts

  const [open, setOpen] = useState(false);
  const [selectedTags, setselectedTags] = useState<string[]>([tag]);
  ...
  return (
    <>
      ...
      <div
        className=" my-2 relative flex items-center gap-2 w-full h-6 p-2 text-sm leading-loose border rounded-lg outline-none focus:border-green-500"
      ...
      <ul className={` m-0 p-0 list-none ${open ? "" : "hidden" } max-h-60 overflow-y-auto border-[.25em] border-solid border-[#777] rounded w-full left-0 top-[calc(100%+0.25em)] z-50`}>
        {uniqueTags.filter((tag) => !selectedTags.includes(tag)).map((option) => (
          <li
            onClick={(e) => {
              e.stopPropagation()
              console.log(option)
      ...
      <ul className="w-full">
        {posts &&
          posts.filter(post => selectedTags.every(e => post.data.tags.includes(e))).map((post) => (
